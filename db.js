...
export async function exportDatabase(){const [cases,experiments,metadata,tombstones,drafts]=await Promise.all([allSetups(),allExperiments(),getAllFrom('metadata'),getAllFrom('tombstones'),getAllFrom('drafts')]);return {format:'xau-library-research-backup',version:2,appVersion:'1.2.0',schemaVersion:2,exportedAt:new Date().toISOString(),timezone:'Asia/Karachi',cases,experiments,metadata,tombstones,drafts}}
...
export async function replaceDatabase(payload){
  ...
  return new Promise((resolve,reject)=>{
    const tx=db.transaction(['setups','experiments','metadata','tombstones','drafts'],'readwrite');
    const s=tx.objectStore('setups'),e=tx.objectStore('experiments'),m=tx.objectStore('metadata'),t=tx.objectStore('tombstones'),d=tx.objectStore('drafts');
    s.clear();e.clear();m.clear();t.clear();d.clear();
    ...
    (payload.drafts||[]).forEach(x=>d.put(x));
